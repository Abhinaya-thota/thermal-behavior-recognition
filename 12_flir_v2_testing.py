...
FRAME_DIR = "data/flir_test_frames"

OUT_PATH = "outputs/flir_sequence_demo2.mp4"
...
MOVE_ENTER_RATIO = 0.12
MOVE_EXIT_RATIO = 0.07
# -----------------------------
# SETUP
# -----------------------------
    ...
    for fname in files:
        ...
        if results and results[0].boxes is not None and results[0].boxes.id is not None:
            ...
            for track_id, xyxy in zip(ids, xyxys):
                ...
                # displacement over wider window
                if len(hist) >= DISPLACEMENT_FRAMES:
                    pts = list(hist)
                    x_old, y_old = pts[-DISPLACEMENT_FRAMES]
                    x_new, y_new = pts[-1]
                    ...
                    displacement = speed
                # normalize motion by person size
                motion_ratio = displacement / box_h
                # hysteresis: harder to switch into moving, easier to remain moving
                prev_behavior = stable_behavior[track_id]
                if prev_behavior == "Moving":
                    if motion_ratio >= MOVE_EXIT_RATIO:
                        raw_behavior = "Moving"
                    ...
                    if motion_ratio >= MOVE_ENTER_RATIO:
                        raw_behavior = "Moving"
                        stationary_start[track_id] = None
                    else:
                        if stationary_start[track_id] is None:
